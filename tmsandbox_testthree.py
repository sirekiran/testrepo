...
try:
   assert(response['CanRelist'] == False)
    ...
    file.write(str(e) + " seen for CanRelist field assertions since expected is True while actual is "+ str(response['CanRelist']) + "\n")
    logging.error(str(e) + " seen for CanRelist field assertions since expected is True while actual is "+ str(response['CanRelist']))
try:
